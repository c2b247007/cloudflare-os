...
/** What calls on a capability whose hosting Durable Object already reset reject with. It arrives
 * flagless — the flagged `durableObjectReset` error only reaches calls in flight at reset time —
 * and so proves the call never reached the object (verified in a workerd probe). Matched by the
 * backend's never-sent retry and the frontend classifier; the frontend canary test pins it. */
export const WORKERD_DEAD_CAPABILITY_MESSAGE =
